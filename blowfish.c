...
void generate_password(char* password) {
	...
	for (i = 0 ; i < PASSWORD_LEN-1 ; i++) {
		...
		if (password[i] == '"' || password[i] == '\\') // We can't use ", because it's used to frame the string in JavaScript ; and \, because it's an espace unsigned character.
			i--;
	}
...
void decode_base64(uint8_t* output, const uint8_t* input) {
	uint8_t chr1, chr2, chr3;
	uint8_t enc1, enc2, enc3, enc4;
	int i = 0;
	
	const char base64_keystr[] = BASE64_KEYSTR;
	while (i < strlen((const char*)input)) {
		enc1 = strchr(base64_keystr, input[i])-base64_keystr;
		i++;
		enc2 = strchr(base64_keystr, input[i])-base64_keystr;
		i++;
		enc3 = strchr(base64_keystr, input[i])-base64_keystr;
		i++;
		enc4 = strchr(base64_keystr, input[i])-base64_keystr;
		i++;
		...
		output[strlen((const char*)output)+1] = '\0'; output[strlen((const char*)output)] = chr1;

		if (enc3 != 64)
			output[strlen((const char*)output)+1] = '\0'; output[strlen((const char*)output)] = chr2;
		if (enc4 != 64)
			output[strlen((const char*)output)+1] = '\0'; output[strlen((const char*)output)] = chr3;
	}
}
...
void num2block32(uint8_t string_out[4], uint32_t num) {
	string_out[0] = num >> 24;
	string_out[1] = num << 8 >> 24;
	string_out[2] = num << 16 >> 24;
	string_out[3] = num << 24 >> 24;
}
void split64by32(uint32_t *output1, uint32_t *output2, const uint8_t block64[8]) {
	uint8_t xL[4]; strncpy((char*)xL, (char*)block64, 4);
	uint8_t xR[4]; strncpy((char*)xR, (char*)block64+4, 4);

	*output1 = block32toNum(xL);
	*output2 = block32toNum(xR);
}
#if 0
void utf8Encode(uint8_t* output, const uint8_t* utftext) {
	int i = 0;
	uint8_t c = 0;
	uint8_t c1 = 0;
	uint8_t c2 = 0;
	...
	while (i < strlen(utftext)) {
		...
		if (c < 128) {
			output[strlen(output)+1] = '\0'; output[strlen(output)] = c;
			i++;
		} else if ((c > 191) && (c < 224)) {
	 		c1 = utftext[i+1];
	 		output[strlen(output)+1] = '\0'; output[strlen(output)] = ((c & 31) << 6) | (c1 & 63);
			i += 2;
		} else {
			c1 = utftext[i+1];
			c2 = utftext[i+2];
			output[strlen(output)+1] = '\0'; output[strlen(output)] = ((c & 15) << 12) | ((c1 & 63) << 6) | (c2 & 63);
			i += 3;
		}
	}
}
...
void decrypt(char* output, const char* key, const char* input) {
	uint8_t string[500] = "";
	decode_base64(string, (const uint8_t*)input);
	
	BLOWFISH_CTX blowfish;
	Blowfish_Init(&blowfish, (uint8_t*)key, strlen(key));
	
	int blocks = ceil(strlen((const char*)string)/8);
	int i;
	uint8_t decryptedString[500] = "";
	for (i = 0 ; i < blocks ; i++) {
		uint8_t block[8];
		strncpy((char*)block, (const char*)string+i*8, 8);
		uint32_t xL, xR;

		split64by32(&xL, &xR, block);

		Blowfish_Decrypt(&blowfish, &xL, &xR);
		
		uint8_t block32[4] = "";
		num2block32(block32, xL);
		strncat((char*)decryptedString, (const char*)block32, 4);
		num2block32(block32, xR);
		strncat((char*)decryptedString, (const char*)block32, 4);
	}
	
	strcpy(output, decryptedString);
	//utf8Encode(output, decryptedString);
}
